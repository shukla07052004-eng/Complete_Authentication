...
function toLineItem(record) {
  ...
  const totalFromFile = Number(record.totalAmount) || 0
  const taxFromFile = Number(record.gstAmount) || 0
  const baseAmount = Number(record.taxableAmount) || (qty && rate ? qty * rate : Math.max(totalFromFile - taxFromFile, 0))
  const taxAmount = Number(record.gstAmount) || Math.round(baseAmount * taxPct / 100)
  const amount = totalFromFile || baseAmount + taxAmount
  return {
    desc: record.itemName || 'Imported line',
    hsn: record.hsnCode || '',
    qty,
    rate,
...
function upsertParty(parties, { name, phone, type, city, address, email, gstin, balance, drCr }) {
...
function buildSalesInvoice(invoiceNo, rows) {
  ...
  return {
    ...
    tax,
    total,
    paid: Number(first.receivedAmount) || (String(first.paymentStatus || '').toLowerCase() === 'paid' ? total : 0),
    status: first.paymentStatus || 'Pending',
...
function buildPurchaseBill(invoiceNo, rows) {
  ...
  return {
    ...
    subtotal,
    tax,
    paid: Number(first.paidAmount) || (String(first.paymentStatus || '').toLowerCase() === 'paid' ? total : 0),
    status: first.paymentStatus || 'Unpaid',
    mode: first.paymentMode || 'Credit',
    items,
    notes: 'Imported from spreadsheet',
  }
}
...
export function mergeImportIntoErpState(state, payload, meta = {}) {
  ...
  invoiceGroups.forEach((rows, invoiceNo) => {
    ...
    rows.forEach((row) => {
      if (row.itemName?.trim()) itemImports += 1
  ...
  if (stockRows.length) {
    stockRows.forEach((row) => {
      itemImports += 1
      items = upsertItem(items, row, { addStockQty: true })
      if (row.partyName?.trim()) {
        partyImports += 1
        parties = upsertParty(parties, { name: row.partyName, phone: row.mobile, type: row.partyType || 'Supplier', city: row.city, address: row.address, email: row.email, gstin: row.gstin })
      }
    })
  }
