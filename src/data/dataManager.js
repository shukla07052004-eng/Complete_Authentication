...
function mergePartyRecords(state, payload = {}, meta = {}) {
  ...
  return {
    ...
    _importStats: { records: records.length, parties: records.filter((record) => record?.partyName).length, invoices: 0, purchases: 0, items: 0 },
  }
}
...
function mergeProductRecords(state, payload = {}, meta = {}) {
  ...
  return {
    ...
    purchases: asArray(state.purchases),
    importMeta: buildImportMeta(payload.metadata, meta),
    _importStats: { records: records.length, items: records.filter((record) => record?.itemName).length, purchases: 0, invoices: 0, parties: 0 },
  }
}
